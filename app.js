...
let depositInfo = () => {
  ...
  depInfo.innerHTML = `Your current balance is : $` + balance.money;
};
...
const numInput =(button)=>{
  if (button === 'enter'){
    let totalNum = document.querySelector(".mainScreen");
    // totalNum.innerHTML = enteredNumb();
    // balance.money += (storedNum*1)
    return '';
  }
  storedNum += button;
  numDisp.innerHTML = storedNum;
